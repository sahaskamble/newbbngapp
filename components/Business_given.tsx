...
export default function Business_given() {
  ...
  const fetchGiven = async () => {
    ...
    try {
      ...
      const res = await fetch(`https://bbmoapp.bbnglobal.net/api/ThankYouSlips/index/received`, {
        method: 'GET',
        headers: myHeaders,
        redirect: 'follow',
      });
          ...
          filteredData?.map((list: any, index: number) => (
            ...
            >
              ...
              <View className='sm:flex justify-between items-start gap-3 my-1'>
                <Text className='text-lg'><Text className='font-bold'>From : </Text>{list.Member?.name}</Text>
